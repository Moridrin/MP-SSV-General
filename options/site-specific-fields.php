...
if (SSV_General::isValidPOST(SSV_General::OPTIONS_ADMIN_REFERER)) {
    if (isset($_POST['reset'])) {
        ...
        if (current_user_can('remove_custom_fields')) {
            $oldNames = $wpdb->get_results("SELECT ID, `name` FROM $baseTable WHERE `shared` = 0");
            ...
            if (!empty($fieldIDs)) {
                ...
                $wpdb->query("DELETE FROM $baseTable WHERE ID NOT IN ($databaseFieldIDs) AND `shared` = 0");
                $wpdb->query("DELETE FROM $customizedTable WHERE `name` IN ($fieldsToBeRemoved)");
            } else {
                $wpdb->query("DELETE FROM $baseTable WHERE 1 AND `shared` = 0");
...
$baseFields = $wpdb->get_results("SELECT * FROM $baseTable WHERE `shared` = 0");
