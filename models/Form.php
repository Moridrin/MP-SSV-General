...
class Form
{
    ...
    /**
     * @param null|array $values if set to null it uses the values from the current user or if the $_GET parameter for member is set the values of that user.
     */
    public function setValues($values = null)
    {
        if ($values == null) {
            if (isset($_GET['member']) && User::isBoard()) {
                $values = User::getByID($_GET['member']);
            } else {
     ...
     */
    public function editorFromPost()
    {
        $form              = new Form();
     ...
     */
    public function loopRecursive($callback)
    {
        /** @var Field $field */
        foreach ($this->fields as $field) {
            $callback($field);
