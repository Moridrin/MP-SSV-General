...
function mp_ssv_print($variable)
{
...
function mp_ssv_get_text_input($title, $id, $value, $type = "text", $args = array())
{
	...
	?>
	<input type="<?php echo $type; ?>" id="<?php echo $object_name; ?>" name="<?php echo $object_name; ?>" value="<?php echo $value; ?>" <?php foreach ($args as $arg) echo $arg; ?>/>
	<?php
	return trim(preg_replace('/\s+/', ' ', ob_get_clean()));
}

function mp_ssv_get_select($title, $id, $selected, $options, $args = array(), $allow_custom = false, $input_type_custom = null)
{
	ob_start();
	if ($allow_custom) {
		$options[] = "Custom";
	...
	<div>
		...
		<input type="text" id="<?php echo $object_custom_name; ?>" name="<?php echo $object_custom_name; ?>" value="<?php echo $input_type_custom; ?>"/>
	</div>
<?php }
...
function mp_ssv_get_checkbox($title, $id, $value, $args = array())
{
	...
	$object_name = $id . "_" . strtolower(str_replace(" ", "_", $title));
	?>
	<br/><input type="checkbox" id="<?php echo $object_name; ?>" name="<?php echo $object_name; ?>" value="yes" <?php if ($value == "yes") : echo "checked"; endif; ?><?php foreach ($args as $arg): echo $arg; endforeach; ?>/>
	<label for="<?php echo $object_name; ?>"><?php echo $title; ?></label>
	<?php
	return trim(preg_replace('/\s+/', ' ', ob_get_clean()));
}
...
function mp_ssv_get_options($parent_id, $options, $type, $args = array())
{
	ob_start();
	?>
	<ul id="<?php echo $parent_id; ?>_options" style="margin: 0;">Options<br/><?php foreach ($options as $option) : echo mp_ssv_get_option($parent_id, $option, $args); endforeach; ?>
		<li>
...
function mp_ssv_get_option($parent_id, $option, $args = array())
{
	...
	if ($option["type"] == "role") {
		...
		<li>
			<!--suppress HtmlFormInputWithoutLabel -->
			<input type="text" id="<?php echo $object_name; ?>_option" name="<?php echo $object_name; ?>_option" value="<?php echo $option["value"]; ?>" <?php foreach ($args as $arg) : echo $arg; endforeach; ?>/>
		</li>
		<?php
	}
...
function mp_ssv_get_role_select($id, $title, $value, $with_title = true, $args = array())
{
	...
	?>
	<select id="<?php echo $object_name; ?>" name="<?php echo $object_name; ?>" <?php foreach ($args as $arg) : echo $arg; endforeach; ?>>
